...
export default function WhatsappPage() {
  ...
  const active = whatsappConversations.find((c) => c.id === activeId);
            ...
            <div className="divide-y divide-ink-200">
              {whatsappConversations.map((c) => (
                <button
                  ...
                  className={cn(
                    "w-full text-left px-4 py-3 hover:bg-white transition",
                    activeId === c.id && "bg-white border-l-2 border-l-brand-600",
                  )}
                >
            ...
            <div className="flex flex-col">
              <header className="px-5 py-3 border-b border-ink-200 flex items-center justify-between bg-white">
                ...
                <div className="flex items-center gap-2">
                  <WaStatusBadge status={active.status} />
                  {active.intent && (
                    <span className="text-[11px] text-ink-700 bg-ink-100 px-2 py-1 rounded">
                      Intencion: {humanLabel(active.intent)}
                    </span>
                  )}
                </div>
              </header>
              ...
              <div className="flex-1 px-5 py-4 space-y-3 overflow-y-auto bg-[radial-gradient(circle_at_top,rgba(46,123,246,0.05),transparent_50%)]">
                {active.messages.map((m) => (
                  ...
                  >
                    <div
                      className={cn(
                        "max-w-[70%] rounded-2xl px-4 py-2.5 text-sm shadow-card",
                        m.direction === "OUT"
                    ...
                    >
                      ...
                      <div>{m.body}</div>
                      <div className={cn("text-[10px] mt-1.5 text-right", m.direction === "OUT" ? "text-brand-100" : "text-ink-400")}>
